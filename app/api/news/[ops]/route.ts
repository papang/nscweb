...
export async function POST(request: NextRequest, { params }: { params: Promise<{ ops: string }> }) {
  ...
  const { 
    srcType, newsId, newsCatId, newsTitle, imgUrl, authorBy, newsContent, 
    srcNews, srcUrl, 
    createdBy, updatedBy, isPublished, isHeadline,
  } = req;
...
async function setHeadline(newsId: number) {
  await db.update(tNews).set({
      isHeadline: 0
    }).where(and(ne(tNews.newsId, newsId), eq(tNews.srcInex, "IN") ));
  const data = await db.update(tNews).set({
      isHeadline: 1
    }).where(eq(tNews.newsId, newsId)).returning({newsId: tNews.newsId, newsTitle: tNews.newsTitle});
}