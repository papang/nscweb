...
export default function NewsDetailDynamic({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Ambil data berita berdasarkan slug URL yang sudah di-unwrap. 
  const currentArticle = newsDatabase[slug] || {
    category: "Berita",
    title: formatSlugToTitle(slug),
    author: "Tim Redaksi NSC",
    date: "Hari Ini",
    readTime: "3 Menit",
    image: "/particle9.webp", // Gambar default jika berita tidak ditemukan
    content: [
      `Ini adalah halaman detail untuk berita mengenai "${formatSlugToTitle(slug)}". Nusantara Star Connect (NSC) terus berkomitmen menghadirkan informasi terkini seputar teknologi satelit LEO dan perkembangannya di seluruh wilayah Indonesia.`,
      "Berbeda dengan infrastruktur tradisional, teknologi terbaru kami menggunakan algoritma canggih untuk mengarahkan energi sinyal tepat ke perangkat pengguna. Hal ini meminimalisir gangguan dan memastikan bandwidth maksimal tersalurkan tanpa hambatan fisik.",
      "Pantau terus portal berita kami untuk mendapatkan wawasan mendalam, tutorial teknis, dan pengumuman resmi terkait layanan internet satelit revolusioner dari Akastar."
    ]
  };
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-6 py-4 border-y border-white/10 text-gray-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-gray-300">{currentArticle.author}</span>
                  </div>
              ...
              >
                ...
                <div className="prose prose-invert max-w-none relative z-10">
                  {currentArticle.content.map((paragraph: string, index: number) => (
                    <div key={index}>
                      <p className="text-base md:text-lg leading-relaxed text-gray-300 mb-8 font-medium">
                        {paragraph}
                      </p>
                    </div>
                  ))}
            ...
            <section className="bg-[#111111] p-6 rounded-[32px] border border-white/5 shadow-2xl backdrop-blur-md">
              ...
              <div className="space-y-6">
                {relatedNews.map((news) => (
                  <Link href={`/berita/detail/${news.title.toLowerCase().replace(/ /g, "-")}`} key={news.id} className="block group cursor-pointer">
