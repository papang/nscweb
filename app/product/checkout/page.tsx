...
export default function CheckoutPage() {
  ...
  const [delskuid, setDelskuid] = useState(0);
                        ...
                        <div key={items.sku_id}>
                          <label
                            ...
                            className={`relative flex flex-col md:flex-row md:items-center md:justify-between gap-3 
                              ...
                              ${(items.prodstream_name == 'MRC Maritim') ? "bg-[#00A8B5]/30" : (
                                (items.prodstream_name == 'MRC Land') ? "bg-[#C2B280]/30" : "bg-[#C2CECE]/10"
                              ) }
                              `}
                          >
                              ...
                              <div>
                                ...
                                <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-slate-600">
                                  <div className="text-white/80"><span className="font-medium text-slate-400 text-xs uppercase block">
                                    {((items.spec_mir_up) ? "Up" : "")}</span> {items.spec_mir_up}</div>
                                  <div className="text-white/80"><span className="font-medium text-slate-400 text-xs uppercase block">
                                    {((items.spec_mir_down) ? "Down" : "")}</span> {items.spec_mir_down}</div>
                            ...
                            <div className="mt-4 md:mt-0 md:text-right border-t md:border-t-0 pt-3 md:pt-0 border-slate-100">
                              <div className="flex justify-end">
                                { !(items.sku_id == "260098" || items.sku_id == "260099") ? (
                                    <DeleteButton 
                                      className="text-orange-300"
                                      onClick={() => {
                                        setDelskuid(items.sku_id);
              ...
              >
                {
                  (isAuthenticated) ? (
                    ...
                    <div className="sticky top-32 bg-[#111111] border border-white/10 rounded-[32px] p-8 shadow-2xl">
                      ...
                      { 
                      (productSum.length>0) ? 
                        productSum.map((items) => (
                          <div className="flex justify-between gap-4 p-4 rounded-2xl bg-white/5 mb-2" key={items.product_name}>
